...
if st.button("Buscar Pacotes & Salvar Automação", type="primary", use_container_width=True):
    ...
    if tipo_voo != "Multidestino" and (not origem_nome or not destino_nome or not orcamento_max):
        ...
        with st.spinner("Salvando pacote no servidor do Monitor de Viagens..."):
            ...
            st.info(f"O robô já gravou as suas premissas. A partir de agora, o nosso motor fará buscas invisíveis todos os dias. Ative o código **{codigo_orcamento}** no menu lateral para começar a receber alertas às **{hora_str}**.")
