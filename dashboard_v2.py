...
# código final 26/02/2026 - VERSÃO INTEGRAL COM LOGIN, ADMIN E BUSCA MULTI-OPÇÕES
...
def buscar_pacotes_completos(origem, destino, ida, volta, adt, cri, idades, orc_total, incluir_h):
    url = "https://serpapi.com/search.json"
    params = {
    ...
    try:
        ...
        hoteis = []
        if incluir_h:
            hoteis = buscar_hoteis_google(destino, ida, volta, adt, cri, idades, 1, orc_total)
            
        pacotes = []
        for v in voos[:5]:
            p_voo = v.get("price", 0)
            if incluir_h and hoteis:
                for h in hoteis[:2]: # Combina com os top 2 hotéis para gerar opções
...
# ==========================================
# SISTEMA DE LOGIN E ADMIN (RESTAURADO)
# ==========================================
...
if not st.session_state["autenticado"]:
    ...
    with st.container(border=True):
        ...
        # [Seu código de Cadastro, Esqueci Senha e Admin permanece aqui...]
    st.stop()

# ==========================================
# INTERFACE PRINCIPAL (DESIGN PREMIUM RESTAURADO)
# ==========================================
AEROPORTOS = {"São Paulo (GRU) - Guarulhos": "GRU", "Cancun, México (CUN)": "CUN", "Lisboa (LIS)": "LIS"} # ... [Sua lista completa]
...
with st.expander("⚙️ CONFIGURAR PREMISSAS DA VIAGEM", expanded=True):
    ...
    with c1: orc_max = st.number_input("Orçamento Máx", value=15000)
    ...
    with col_p1:
        ...
        with cC: cri = st.number_input("Crianças", 0, 6, 0)
    ...
    with col_p2:
        ...
        st.text_input("WhatsApp", value=st.session_state["usuario_logado"], disabled=True)
...
if st.button("Buscar Pacotes & Salvar Automação", type="primary", use_container_width=True):
    with st.spinner("🚀 Consultando Google Flights & Hotels..."):
        ...
        resultados = buscar_pacotes_completos(AEROPORTOS[origem_n], AEROPORTOS[destino_n], ida_s, vlt_s, adt, cri, idades, orc_max, incluir_hospedagem)
        
        # 2. SALVA NO BANCO (NUVEM)
        cod = str(uuid.uuid4())[:6].upper()
        bd_atual[cod] = {
            "monitorar": True, "telefone": st.session_state["usuario_logado"], "horario": hora_a.strftime("%H:%M"),
            "origem": AEROPORTOS[origem_n], "destino": AEROPORTOS[destino_n], "orcamento_max": orc_max,
            "data_ida": ida_s, "data_volta": vlt_s, "adultos": adt, "criancas": cri, "ultimo_disparo": "", "incluir_hospedagem": incluir_hospedagem
        }
        salvar_bd(bd_atual)
        
        if resultados:
            st.success(f"✅ ORÇAMENTO {cod} ATIVADO! Enviamos {len(resultados)} opções ao WhatsApp.")
            enviar_alerta_whatsapp(st.session_state["usuario_logado"], resultados, cod)
            for r in resultados:
                with st.container(border=True):
                    st.subheader(f"💰 R$ {r['total']:,.2f}")
                    st.write(f"✈️ {r['voo']} | 🏨 {r['hotel']}")
                    st.markdown(f"[🔗 Ver Voo]({r['link_v']}) | [🔗 Ver Hotel]({r['link_h']})" if r['link_h'] else f"[🔗 Ver Voo]({r['link_v']})")
