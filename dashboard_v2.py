...
st.set_page_config(page_title="Monitor Tucano Verde", layout="wide")
...
if not st.session_state["autenticado"]:
    st.title("🔒 Acesso Restrito - Tucano Verde")
...
# ==========================================
st.sidebar.title("🤖 Painel Tucano Verde")
...
st.title("✈️ Monitor de Viagens Tucano Verde")
...
if st.button("Buscar Pacotes & Salvar Automação", type="primary", use_container_width=True):
    if tipo_voo != "Multidestino" and (not origem_nome or not destino_nome or not orcamento_max):
        ...
        with st.spinner("Salvando pacote no servidor da Tucano Verde..."):
            ...
            st.info(f"O robô da Tucano Verde já gravou as suas premissas. A partir de agora, o nosso motor fará buscas invisíveis todos os dias. Ative o código **{codigo_orcamento}** no menu lateral para começar a receber alertas às **{hora_str}**.")
            st.warning("⚠️ Nota: Os resultados demoram até 24h para o primeiro disparo automático chegar ao seu celular pelo motor de fundo.")# código final 20/01/2026
# dashboard_v2.py
import streamlit as st
import datetime
import requests
...
if st.button("Buscar Pacotes & Salvar Automação", type="primary", use_container_width=True):
    if tipo_voo != "Multidestino" and (not origem_nome or not destino_nome or not orcamento_max):
        ...
        with st.spinner("Salvando pacote no servidor da Tucano Verde..."):
            ...
            st.success(f"🎉 **✅ ORÇAMENTO SALVO COM SUCESSO! CÓDIGO: {codigo_orcamento}**")
            st.info(f"O robô da Tucano Verde já gravou as suas premissas. A partir de agora, o nosso motor fará buscas invisíveis todos os dias. Ative o código **{codigo_orcamento}** no menu lateral para começar a receber alertas às **{hora_str}**.")
            st.warning("⚠️ Nota: Os resultados demoram até 24h para o primeiro disparo automático chegar ao seu celular pelo motor de fundo.")
